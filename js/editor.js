(() => {
  ...
  function renderFolderElements(){
    ...
    const list=(elementManifest||[])
      .filter(el=>cat==="all"||el.category===cat)
      .filter(el=>!q || JSON.stringify({n:el.name,c:el.category,s:el.src}).toLowerCase().includes(q));

    if(!list.length){
      box.innerHTML='<div class="asset-empty">Nessun elemento trovato. Carica PNG/SVG nelle cartelle GitHub e premi “Scansiona libreria”.</div>';
      return;
    }

    box.innerHTML=list.map(el=>`<div class="asset-card">
      ...
      <button class="green" data-folder-element="${escHtml(el.id)}">Aggiungi</button>
    </div>`).join("");

    box.querySelectorAll("[data-folder-element]").forEach(b=>b.onclick=()=>{
      const el=elementManifest.find(x=>String(x.id)===String(b.dataset.folderElement));
      if(el) addGraphic(el.type||"image",el.name||el.id,el.src||"");
    });
  }
