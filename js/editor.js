(() => {
  ...
  function renderFolderElements(){
    ...
    if(!list.length){
      box.innerHTML='<p class="hint">Nessun elemento trovato. Carica PNG/SVG nelle cartelle oppure premi “Scansiona cartelle GitHub”.</p>';
      return;
    }
    box.innerHTML=list.map(el=>`<div class="folder-element-item"><h3>${escHtml(el.name||el.id||"Elemento")}</h3><span class="tag">${escHtml(el.category||"decorazioni")}</span>${el.src?`<img src="${escHtml(el.src)}">`:""}<small>${escHtml(el.src||"")}</small><button class="green" data-folder-element="${escHtml(el.id)}">Aggiungi</button></div>`).join("");
  ...
  async function scanGithubElements(){
    try{
      ...
      elementManifest=els;
      renderFolderElements();
      alert("Elementi trovati nelle cartelle GitHub: "+els.length);
    }catch(e){
