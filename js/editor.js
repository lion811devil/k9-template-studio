(() => {
  ...
  const $ = id => {
    ...
    if (el) return el;
    // Elemento mancante: creo un fallback invisibile per evitare blocchi dell'editor.
  ...
  const ELEMENT_FAV_LS = "k9-template-studio-v7-element-favs";
  const ELEMENT_RECENT_LS = "k9-template-studio-v7-element-recents";
  ...
  let graphicFileData = ""; let graphicFileList=[];
  ...
  function def(){
    return {
      id:"p-"+Date.now(),
      name:"Nuovo certificato",certTitle:"ATTESTATO",certSubtitle:"DI PARTECIPAZIONE",signatureLabel:"Ente formatore",
      ...
      offsets:{
        title:{x:0,y:0},subtitle:{x:0,y:0},names:{x:0,y:0},text:{x:0,y:0},course:{x:0,y:0},date:{x:0,y:0},signatureLabel:{x:0,y:0},signature:{x:0,y:0}
      },
      textStyle:{
        title:{size:54,color:"#123d28"},subtitle:{size:32,color:"#123d28"},names:{size:58,color:"#123d28"},text:{size:28,color:"#8b6416"},course:{size:34,color:"#123d28"},date:{size:22,color:"#111111"},signatureLabel:{size:22,color:"#111111"},signature:{size:26,color:"#111111"}
      }
    };
  }
  ...
  function sync(){
    ...
    p.certTitle = $("certTitle").value.trim() || "ATTESTATO";
    p.certSubtitle = $("certSubtitle").value.trim() || "DI PARTECIPAZIONE";
    p.signatureLabel = $("signatureLabel").value.trim() || "Ente formatore";
  ...
  function fill(){
    ...
    if(!p.certTitle)p.certTitle="ATTESTATO"; if(!p.certSubtitle)p.certSubtitle="DI PARTECIPAZIONE"; if(!p.signatureLabel)p.signatureLabel="Ente formatore";
    $("certTitle").value = p.certTitle;
    $("certSubtitle").value = p.certSubtitle;
    $("signatureLabel").value = p.signatureLabel;
    const titlePair = p.certTitle+"|"+p.certSubtitle;
    const titleSelect = $("titlePreset");
    if([...titleSelect.options].some(o=>o.value===titlePair)) titleSelect.value=titlePair; else titleSelect.value="custom";
    const sigSelect = $("signatureLabelPreset");
    if([...sigSelect.options].some(o=>o.value===p.signatureLabel)) sigSelect.value=p.signatureLabel; else sigSelect.value="custom";
  ...
  function textValue(id){
    if(id==="title") return p.certTitle || "ATTESTATO";
    if(id==="subtitle") return p.certSubtitle || "DI PARTECIPAZIONE";
    if(id==="signatureLabel") return p.signatureLabel || "Ente formatore";
  ...
  function draw(){
    ...
    ["title","subtitle","names","text","course","date","signatureLabel","signature"].forEach(drawText);
    drawQr();
  ...
  function addGraphic(type,name,src){
    ...
    p.graphics.push(g); refreshGraphicOptions(g.id); renderLayers(); draw();
  }
  ...
  function textCtl(){
    ...
    if(s){ $("textSize").value=s.size; $("textColor").value=s.color; if($("textColorHex"))$("textColorHex").value=s.color; }
  ...
  async function setProject(x){
    ...
    p.offsets=p.offsets||{}; if(!p.offsets.signatureLabel)p.offsets.signatureLabel={x:0,y:0};
    p.textStyle=p.textStyle||{}; if(!p.textStyle.signatureLabel)p.textStyle.signatureLabel={size:22,color:"#111111"};
    fill(); await reload(); refreshGraphicOptions(); renderLayers(); draw();
  }

  function newProject(){ p=def(); img={base:null,subject:null,logo:null}; qrCanvas=null; qrPayloadCache=""; fill(); refreshGraphicOptions(); renderLayers(); draw(); }
  ...
  async function loadElementManifest(){
    try{
      const r=await fetch("assets/elements/elements.json?ts="+Date.now());
      if(!r.ok) throw new Error();
      const data=await r.json();
      elementManifest=Array.isArray(data)?data:(data.elements||[]);
    }catch(e){elementManifest=[]}
  }
  
  function elementFavs(){try{return JSON.parse(localStorage.getItem(ELEMENT_FAV_LS)||"[]")}catch{return[]}}
  function saveElementFavs(x){localStorage.setItem(ELEMENT_FAV_LS,JSON.stringify(x))}
  function elementRecents(){try{return JSON.parse(localStorage.getItem(ELEMENT_RECENT_LS)||"[]")}catch{return[]}}
  function saveElementRecents(x){localStorage.setItem(ELEMENT_RECENT_LS,JSON.stringify(x.slice(0,40)))}
  function rememberElement(el){let r=elementRecents().filter(x=>x.id!==el.id);r.unshift(el);saveElementRecents(r)}
  function toggleFavElement(el){let f=elementFavs();let i=f.findIndex(x=>x.id===el.id);if(i>=0)f.splice(i,1);else f.unshift(el);saveElementFavs(f);renderFolderElements()}
  function currentElementList(){
    const cat=($("graphicCategory")&&$("graphicCategory").value)||"all";
    if(cat==="preferiti")return elementFavs();
    if(cat==="recenti")return elementRecents();
  ...
  function renderFolderElements(){
    ...
    const cat=($("graphicCategory")&&$("graphicCategory").value)||"all";
    const q=($("elementSearch")&&$("elementSearch").value||"").toLowerCase().trim();
    const favs=elementFavs();
    const source=currentElementList();
    const list=(source||[])
      .filter(el=>cat==="all"||cat==="preferiti"||cat==="recenti"||el.category===cat)
      .filter(el=>!q || JSON.stringify({n:el.name,c:el.category,s:el.src}).toLowerCase().includes(q));

    if(!list.length){
      box.innerHTML='<div class="asset-empty">Nessun elemento trovato. Carica PNG/SVG nelle cartelle GitHub e premi “Scansiona”.</div>';
      return;
    }

    box.innerHTML=list.map(el=>{
      const isFav=favs.some(f=>f.id===el.id);
      return `<div class="asset-card">
        <button class="fav ${isFav?"active":""}" data-fav-element="${escHtml(el.id)}">★</button>
        <div class="asset-thumb">${el.src?`<img src="${escHtml(el.src)}" loading="lazy">`:"<span>Elemento</span>"}</div>
        <h3>${escHtml(el.name||el.id||"Elemento")}</h3>
        <small>${escHtml(el.category||"decorazioni")}<br>${escHtml(el.src||"")}</small>
        <div class="add-row"><button class="green" data-folder-element="${escHtml(el.id)}">Aggiungi</button><button data-preview-element="${escHtml(el.id)}">👁</button></div>
      </div>`;
    }).join("");

    box.querySelectorAll("[data-folder-element]").forEach(b=>b.onclick=()=>{
      const el=list.find(x=>String(x.id)===String(b.dataset.folderElement));
      if(el){ addGraphic(el.type||"image",el.name||el.id,el.src||""); rememberElement(el); }
    });
    box.querySelectorAll("[data-fav-element]").forEach(b=>b.onclick=()=>{
      const el=list.find(x=>String(x.id)===String(b.dataset.favElement));
      if(el)toggleFavElement(el);
    });
    box.querySelectorAll("[data-preview-element]").forEach(b=>b.onclick=()=>{
      const el=list.find(x=>String(x.id)===String(b.dataset.previewElement));
      if(el&&el.src)window.open(el.src,"_blank","noopener,noreferrer");
    });
  }
  function filterGraphicButtons(){
    const cat=($("graphicCategory")&&$("graphicCategory").value)||"all";
    document.querySelectorAll("[data-add-graphic]").forEach(b=>b.classList.toggle("hidden-cat",!(cat==="all"||cat==="preferiti"||cat==="recenti"||b.dataset.cat===cat)));
    document.querySelectorAll("#categoryChips [data-cat]").forEach(b=>b.classList.toggle("active",b.dataset.cat===cat));
  ...
  function inferGithubRepo(){
    ...
    if(host.endsWith(".github.io")){
      ...
      const repo=path[0] || "k9-template-studio";
      return {owner,repo,branch:"main"};
    }
    return {owner:"lion811devil",repo:"k9-template-studio",branch:"main"};
  }
  function categoryFromPath(path){
    const parts=String(path||"").split("/");
    const idx=parts.indexOf("elements");
    return idx>=0 && parts[idx+1] ? parts[idx+1] : "decorazioni";
  }

  async function githubListRecursive(apiUrl, out=[]){
    const r=await fetch(apiUrl);
    if(!r.ok) throw new Error("GitHub API non raggiungibile");
    const items=await r.json();
    for(const it of items){
      if(it.type==="dir") await githubListRecursive(it.url, out);
      else if(/\.(png|jpg|jpeg|webp|svg)$/i.test(it.name)) out.push(it);
    }
    return out;
  }

  async function scanGithubElements(){
    try{
      const repo=inferGithubRepo();
      const api=`https://api.github.com/repos/${repo.owner}/${repo.repo}/contents/assets/elements?ref=${repo.branch}`;
      const files=await githubListRecursive(api,[]);
      const els=files.map((f,i)=>{
        const src=f.path;
        const cat=categoryFromPath(f.path);
        return {id:"git-"+i+"-"+f.name.replace(/\W+/g,"-"),name:f.name.replace(/\.[^.]+$/,""),category:cat,type:"image",src};
      });
      elementManifest=els;
      renderFolderElements();
      alert("Elementi caricati nella libreria: "+els.length);
    }catch(e){
      alert("Scansione non riuscita. Controlla che la repository sia pubblica e che assets/elements esista.");
    }
  }

  function showFolderHelp(){
    alert("Carica gli elementi qui:\\n\\nassets/elements/cornici\\nassets/elements/coccarde\\nassets/elements/timbri\\nassets/elements/fregi\\nassets/elements/separatori\\nassets/elements/sigilli\\nassets/elements/nastri\\nassets/elements/stemmi\\nassets/elements/icone\\nassets/elements/loghi\\nassets/elements/decorazioni\\nassets/elements/sfondi\\n\\nFormati consigliati: PNG trasparente o SVG.");
  }

  
  function selectedObj(){
    const id=$("selectedElement").value;
    if(id&&id.startsWith("graphic:"))return graphicById(id.split(":")[1]);
    if(id==="photo")return p.photo;
    if(id==="logo")return p.logo;
    if(id==="qr")return p.qr;
    if(p.offsets[id])return pos(id);
    return null;
  }
  function renderLayers(){
    const box=$("layerList"); if(!box)return;
    const fixed=["photo","logo","qr","title","subtitle","names","text","course","date","signatureLabel","signature"];
    const rows=[...(p.graphics||[]).map(g=>({id:"graphic:"+g.id,name:g.name||g.type})),...fixed.map(id=>({id,name:id}))];
    const selected=$("selectedElement").value;
    box.innerHTML=rows.map(r=>`<div class="layer-item ${r.id===selected?"active":""}"><button data-layer-select="${escHtml(r.id)}">${escHtml(r.name)}</button><button data-layer-up="${escHtml(r.id)}">↑</button><button data-layer-down="${escHtml(r.id)}">↓</button></div>`).join("");
    box.querySelectorAll("[data-layer-select]").forEach(b=>b.onclick=()=>{$("selectedElement").value=b.dataset.layerSelect;textCtl();renderLayers()});
    box.querySelectorAll("[data-layer-up]").forEach(b=>b.onclick=()=>{if(b.dataset.layerUp.startsWith("graphic:")){$("selectedElement").value=b.dataset.layerUp;moveGraphicLayer(1);renderLayers()}});
    box.querySelectorAll("[data-layer-down]").forEach(b=>b.onclick=()=>{if(b.dataset.layerDown.startsWith("graphic:")){$("selectedElement").value=b.dataset.layerDown;moveGraphicLayer(-1);renderLayers()}});
  }
  function fillProps(){
    const id=$("selectedElement").value, o=selectedObj();
    if(!$("propX")||!o)return;
    $("propX").value=Math.round(o.x||0);$("propY").value=Math.round(o.y||0);
    $("propScale").value=Math.round(o.scale||100);$("propRotate").value=Math.round(o.rotate||0);$("propOpacity").value=Math.round(o.opacity||100);
    renderLayers();
  }
  function applyProps(){
    const id=$("selectedElement").value;
    let o=null;
    if(id&&id.startsWith("graphic:"))o=graphicById(id.split(":")[1]);
    else if(id==="photo")o=p.photo; else if(id==="logo")o=p.logo; else if(id==="qr")o=p.qr;
    if(!o)return;
    o.x=+$("propX").value;o.y=+$("propY").value;o.scale=+$("propScale").value;o.rotate=+$("propRotate").value;o.opacity=+$("propOpacity").value;
    fill();draw();
  }
  function duplicateSelected(){
    const id=$("selectedElement").value;
    if(id&&id.startsWith("graphic:")){
      const g=graphicById(id.split(":")[1]); if(!g)return;
      const c=JSON.parse(JSON.stringify(g)); delete c._img; c.id="g-"+Date.now()+"-"+Math.random().toString(36).slice(2,5); c.name=(c.name||"Elemento")+" copia"; c.x+=30; c.y+=30; p.graphics.push(c); refreshGraphicOptions(c.id); draw();
    }
  }
  ...
  function bind(){
    ...
    const freeSources={
      openFreeSvg:"https://freesvg.org/search/?query=certificate",
      openSvgRepo:"https://www.svgrepo.com/vectors/certificate/",
      openOpenclipart:"https://openclipart.org/search/?query=certificate",
      openWikimedia:"https://commons.wikimedia.org/w/index.php?search=certificate+frame+svg&title=Special:MediaSearch&type=image",
      openVecteezyFree:"https://www.vecteezy.com/free-vector/certificate-border",
      openFreepikFree:"https://www.freepik.com/search?format=search&query=certificate%20border"
    };
    Object.keys(freeSources).forEach(id=>{if($(id))$(id).onclick=()=>window.open(freeSources[id],"_blank","noopener,noreferrer")});

    
    if($("quickAddElement"))$("quickAddElement").onclick=()=>{document.querySelectorAll(".tab,.tab-panel").forEach(x=>x.classList.remove("active"));document.querySelector('[data-tab="elementi"]').classList.add("active");$("tab-elementi").classList.add("active")};
    if($("quickLayers"))$("quickLayers").onclick=()=>{document.querySelectorAll(".tab,.tab-panel").forEach(x=>x.classList.remove("active"));document.querySelector('[data-tab="muovi"]').classList.add("active");$("tab-muovi").classList.add("active");renderLayers()};
    if($("quickProps"))$("quickProps").onclick=()=>{document.querySelectorAll(".tab,.tab-panel").forEach(x=>x.classList.remove("active"));document.querySelector('[data-tab="muovi"]').classList.add("active");$("tab-muovi").classList.add("active");fillProps()};
    if($("quickFit"))$("quickFit").onclick=()=>{$("selectedElement").value="photo";p.photo={x:320,y:580,scale:100,opacity:100,feather:30,rotate:0,effect:"soft"};fill();draw()};
    document.querySelectorAll("#categoryChips [data-cat]").forEach(b=>b.onclick=()=>{ $("graphicCategory").value=b.dataset.cat; filterGraphicButtons(); });
    if($("elementSearch"))$("elementSearch").oninput=renderFolderElements;
    if($("applyProps"))$("applyProps").onclick=applyProps;
    if($("duplicateSelected"))$("duplicateSelected").onclick=duplicateSelected;
    if($("lockSelected"))$("lockSelected").onclick=()=>alert("Blocco/sblocco sarà operativo nella prossima ottimizzazione: al momento usa livelli e proprietà.");
    ...
    ["projectName","certTitle","certSubtitle","signatureLabel","clientName","dogName","courseCustom","mainText","instructor","qrText"].forEach(id=>$(id).addEventListener("input",draw));
    $("titlePreset").onchange=()=>{ if($("titlePreset").value!=="custom"){ const parts=$("titlePreset").value.split("|"); $("certTitle").value=parts[0]||"ATTESTATO"; $("certSubtitle").value=parts[1]||""; draw(); } };
    $("signatureLabelPreset").onchange=()=>{ if($("signatureLabelPreset").value!=="custom"){ $("signatureLabel").value=$("signatureLabelPreset").value; draw(); } };
    ...
    $("textColor").oninput=()=>{ const id=$("selectedElement").value; if(p.textStyle[id]){p.textStyle[id].color=$("textColor").value; if($("textColorHex"))$("textColorHex").value=$("textColor").value;} draw(); };
    if($("textColorHex"))$("textColorHex").oninput=()=>{ const v=$("textColorHex").value.trim(); if(/^#[0-9a-fA-F]{6}$/.test(v)){ const id=$("selectedElement").value; if(p.textStyle[id]){p.textStyle[id].color=v; $("textColor").value=v;} draw(); } };
    document.querySelectorAll("[data-color]").forEach(b=>b.onclick=()=>{ const id=$("selectedElement").value; if(p.textStyle[id]){p.textStyle[id].color=b.dataset.color; $("textColor").value=b.dataset.color; if($("textColorHex"))$("textColorHex").value=b.dataset.color; draw(); } });
    ...
    if($("scanGithubElements"))$("scanGithubElements").onclick=scanGithubElements;
    if($("showFolderHelp"))$("showFolderHelp").onclick=showFolderHelp;
    ...
    if($("graphicFile")) $("graphicFile").onchange=async e=>{ graphicFileList=[...e.target.files]; graphicFileData=graphicFileList[0]?await read(graphicFileList[0]):""; if(!$("graphicName").value&&graphicFileList[0])$("graphicName").value=graphicFileList[0].name.replace(/\.[^.]+$/,""); };
    if($("addGraphicImage")) $("addGraphicImage").onclick=async()=>{ if(graphicFileList&&graphicFileList.length){ for(const f of graphicFileList){ const data=await read(f); addGraphic("image",f.name.replace(/\.[^.]+$/,""),data); } graphicFileList=[]; graphicFileData=""; $("graphicName").value=""; return; } if(!graphicFileData)return alert("Prima scegli un PNG/SVG."); addGraphic("image",($("graphicName").value||"PNG personale").trim(),graphicFileData); graphicFileData=""; $("graphicName").value=""; };
    if($("deleteSelectedGraphic")) $("deleteSelectedGraphic").onclick=()=>{ const id=$("selectedElement").value; if(!id.startsWith("graphic:"))return alert("Seleziona prima un elemento grafico nella scheda Muovi."); p.graphics=(p.graphics||[]).filter(g=>g.id!==id.split(":")[1]); refreshGraphicOptions(); renderLayers(); draw(); };
  ...
  window.K9Editor={
    init: async function(){
      try { return await init(); }
        ...
        console.error("K9Editor init failed:", err);
        const s = document.getElementById("status");
        if(s) s.textContent = "Editor caricato con correzione sicurezza. Alcune funzioni avanzate potrebbero richiedere ricarica.";
        try { canvas=document.getElementById("canvas"); ctx=canvas.getContext("2d"); draw(); } catch(e) {}
      }
    },
    draw,setProject,getProject,newProject
  };
})();
